    ...
    def run_AM_AW(self):
        ...
        for move_count in range(AM_count):
            ...
            print('change_counts', move_count, 'Test_L2error:', '{0:.2e}'.format(error), 'Loss-pde', '{0:.2e}'.format(loss_e.item()),
                                  ...
                                  'xfs', '{0:.2e}'.format(self.x_f_s.item()),
                                  'xfslabel', '{0:.2e}'.format(self.x_label_s.item()))
            self.x_test_estimate_collect.append([move_count, '{0:.2e}'.format(error)])
...
if __name__ == '__main__':
    ...
    if os.path.exists(r'D:\文稿\发表论文\2024MSSP\实验数据\20241230\save'):
        net = torch.load(
            r'D:\文稿\发表论文\2024MSSP\实验数据\20241230\save\model_RAMAW-A3S150C5-PINN.pt2024-12-30-21-55-18')
