...
def draw_Ady_exact_points(points, move_count, pic_name, N_points=None, show_exact=False):
    ...
    if show_exact:
        ...
        # plt.colorbar()
        # plt.title(r'Exact $Adx$')
        model.render_exact_residual_Ady_points()
    if N_points is not None:
        adds = N_points.cpu().detach().numpy()
        plt.plot(adds[:, [0]], adds[:, [1]], 'kx', markersize=4)

    filename = f"{pic_name}1-{move_count}-Ady.png"
    points = points.cpu().detach().numpy()
    plt.plot(points[:, [0]], points[:, [1]], 'rx', markersize=4)
    plt.xlim(length_left, length_right)
    plt.ylim(thickness_mag + thickness_air, thickness_mag + thickness_air + thickness_copper)
...
if __name__ == '__main__':
    ...
    # 设置模型权重调节类型
    model_type = 4  # 0:baseline  1:AW  2:AM  3:AM_AW  4:AM_sensors
    # 采样方式设置
    AM_type = 2  # 0:RAM  1:WAM  #2:None
    AM_K = 1
    AM_count = 20
    AW_lr = 0.001
    pic_name = "IDW-PINN_A5S150C5"
    model_name = "model_IDW-PINN-A5S150C5.pt"
    ...
    if os.path.exists(r'D:\文稿\发表论文\2024MSSP\实验数据\20241230\save'):
        net = torch.load(
            r'D:\文稿\发表论文\2024MSSP\实验数据\20241230\save\model_IDW-PINN-A3S150C5.pt2024-12-30-21-00-40')
        print("load pervious model success")
