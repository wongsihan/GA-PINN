...
from utils_idw import is_cuda, tensor, device
from physics_idw import draw_Adx_exact_points, draw_Ady_exact_points
...
class Model:
    ...
    def cal_exact_torque(self,x1,x2):
        from config_idw import length_left, length_right, field_res, thickness_mag, thickness_air, thickness_copper, sigma, omega, Rm, model_type, AM_count, lbgfs_lr, adam_lr, adam_iter, lbgfs_iter, AW_lr, AM_type, pic_name, L
        from physics_idw import exact_A2
    ...
    def render_exact_residual_Adx_points(self, move_count=0, pic_name="GAM-Adx residual points"):
        from config_idw import length_left, length_right, field_res, thickness_mag, thickness_air, thickness_copper, sigma, omega, Rm, model_type, AM_count, lbgfs_lr, adam_lr, adam_iter, lbgfs_iter, AW_lr, AM_type, pic_name, L
        from physics_idw import exact_A2
        x_draw, y_draw = np.mgrid[length_left:length_right:field_res * 1j,
                         thickness_mag + thickness_air:thickness_mag + thickness_air + thickness_copper:field_res * 1j]
        ...
        import os
        os.makedirs("results/idw", exist_ok=True)
        filename = f"results/idw/{pic_name}1-{move_count}.png"
        plt.title(filename)
        plt.savefig(filename)
        
    def render_exact_residual_Ady_points(self, move_count=0, pic_name="GAM-Ady residual points"):
        from config_idw import length_left, length_right, field_res, thickness_mag, thickness_air, thickness_copper, sigma, omega, Rm, model_type, AM_count, lbgfs_lr, adam_lr, adam_iter, lbgfs_iter, AW_lr, AM_type, pic_name, L
        from physics_idw import exact_A2
        x_draw, y_draw = np.mgrid[length_left:length_right:field_res * 1j,
                         thickness_mag + thickness_air:thickness_mag + thickness_air + thickness_copper:field_res * 1j]
    ...
    def render_res(self, move_count, pic_name):
        ...
        os.makedirs("results/idw", exist_ok=True)
        filename = f"results/idw/{pic_name}1-{move_count}.png"
        filename2 = f"results/idw/{pic_name}2-{move_count}.png"
    ...
    def render_residual(self, move_count, pic_name='residual'):
        ...
        plt.xlabel('x', fontsize=14, fontname='Times New Roman')
        plt.ylabel('y', fontsize=14, fontname='Times New Roman')
        filename = f"results/idw/{pic_name}1-{move_count}-residual.png"
        filename2 = f"results/idw/{pic_name}2-{move_count}-residual.png"
    ...
    def run_AM_sensors(self):
        ...
        for move_count in range(AM_count):
            ...
            self.render_res(move_count, pic_name)
            self.render_residual(move_count, pic_name)
            draw_Adx_exact_points(self.x_f_M_A2, move_count, pic_name, N_points=self.x_f_N_A2, show_exact=True)
            draw_Ady_exact_points(self.x_f_M_A2, move_count, pic_name, N_points=self.x_f_N_A2, show_exact=True)
