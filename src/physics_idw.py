...
from utils_idw import is_cuda, tensor

# 计算A1区域损失的函数
def x_f_loss_fun_A1(x, train_U):
    from config_idw import br, Pi, u0, alpha, beta
...
def exact_A1(x,y):
    from config_idw import beta, K, b, c, d
...
def draw_Adx_exact_points(points, move_count, pic_name, N_points=None, show_exact=False):
    from config_idw import length_left, length_right, field_res, thickness_mag, thickness_air, thickness_copper, sigma, omega, Rm, beta, K, b, c, d, br, Pi, u0, alpha
    ...
    if show_exact:
        ...
        # model.render_exact_residual_Adx_points()  # 注释掉，因为model未定义
        pass  # 添加pass语句
    ...
    filename = f"results/idw/{pic_name}1-{move_count}-Adx.png"
...
def draw_Ady_exact_points(points, move_count, pic_name, N_points=None, show_exact=False):
    ...
    if show_exact:
        ...
        # plt.colorbar()
        # plt.title(r'Exact $Adx$')
        # model.render_exact_residual_Ady_points()  # 注释掉，因为model未定义
        pass  # 添加pass语句
    if N_points is not None:
        adds = N_points.cpu().detach().numpy()
        plt.plot(adds[:, [0]], adds[:, [1]], 'kx', markersize=4)

    filename = f"results/idw/{pic_name}1-{move_count}-Ady.png"
    points = points.cpu().detach().numpy()
    plt.plot(points[:, [0]], points[:, [1]], 'rx', markersize=4)
    plt.xlim(length_left, length_right)
    plt.ylim(thickness_mag + thickness_air, thickness_mag + thickness_air + thickness_copper)
