...
from utils_idw import is_cuda, tensor, random_fun, device
from model_idw import Net, Model
from physics_idw import x_f_loss_fun_A1, x_f_loss_fun_A2, loss_b1, loss_b2, loss_b3, loss_b4, loss_b57, loss_b68, exact_A1, exact_A2
from config_idw import *
...
def main():
    ...
    # bc data边界上的点
    # A1左侧
    x1_boundary_left_A1 = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), torch.full([Nbc, 1], height_bottom, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * thickness_mag), dim=1) #leftA1
    # A2左侧
    x1_boundary_left_A2 = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), torch.full([Nbc, 1], thickness_mag, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (thickness_air+thickness_copper)), dim=1) #leftA2
    # A1右侧
    x1_boundary_right_A1 = torch.cat((torch.full([Nbc, 1], length_right, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), torch.full([Nbc, 1], height_bottom, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * thickness_mag), dim=1) #rightA1
    # A2右侧
    x1_boundary_right_A2 = torch.cat((torch.full([Nbc, 1], length_right, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), torch.full([Nbc, 1], thickness_mag, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (thickness_air+thickness_copper)), dim=1) #rightA2
    # A1下侧
    x2_boundary_left = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (length_right-length_left), torch.full([Nbc, 1], height_bottom, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))), dim=1) #bottom
    # A2上侧
    x2_boundary_right = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (length_right-length_left), torch.full([Nbc, 1], height_top, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))), dim=1) #top
    # 图中的边界2
    x3_boundary_one = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (length_right-length_left), torch.full([Nbc, 1], (thickness_mag+thickness_air), device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))), dim=1) #middle top
    # A1与A2的交界
    x3_boundary_two = torch.cat((torch.full([Nbc, 1], length_left, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) + torch.rand([Nbc, 1], device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) * (length_right-length_left), torch.full([Nbc, 1], thickness_mag, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))), dim=1) # middle bottom
    ...
    # 检查是否有预训练模型
    model_path = "models/model_IDW-PINN.pt"
